...
The converter process, which extracts keywords and metadata from HTML documents, has been optimized to run about
...
### RDRPosTagger 
The `RDRPosTagger` which does Part Of Speech tagging, which was already impressively fast, already aggressively modified to be faster,
has been further optimized to be faster still, and its Java object tree design was replaced with flat integer arrays. 
